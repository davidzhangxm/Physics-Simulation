...
AABB::AABB(const glm::vec3& center, float_t radius)
{
  setNull();
...
void AABB::extend(const glm::vec3& p, float_t radius)
{
  glm::vec3 r(radius);
  if (!isNull())
  {
...
void AABB::extendDisk(const glm::vec3& c, const glm::vec3& n, float_t r)
{
  if (glm::length(n) < 1.e-12) { extend(c); return; }
  glm::vec3 norm = glm::normalize(n);
  float_t x = sqrt(1 - norm.x) * r;
  float_t y = sqrt(1 - norm.y) * r;
  float_t z = sqrt(1 - norm.z) * r;
...
float_t AABB::getLongestEdge() const
{
  return glm::compMax(getDiagonal());
}

float_t AABB::getShortestEdge() const
{
...
bool AABB::isSimilarTo(const AABB& b, float_t diff) const
{
  if (isNull() || b.isNull()) return false;

  glm::vec3 acceptable_diff=( (getDiagonal()+b.getDiagonal()) / float_t(2.0))*diff;
